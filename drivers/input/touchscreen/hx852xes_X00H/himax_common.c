...
extern void himax_ic_reset(uint8_t loadconfig,uint8_t int_off);
	...
	{
		#include "HQ_QL1526_ASUS_CTPFW_HLT_HX8527E44_DVT_0F.0A.0A.i"
	};
#endif
...
#if defined(HX_TP_PROC_SELF_TEST)|| defined(CONFIG_TOUCHSCREEN_HIMAX_ITO_TEST_X00H)
...
int Hxtp_tpd_usb_plugin(int charger_on)
{
	USB_detect_flag = charger_on;
	I("[HXTP] %s, charger plug %s", __func__, (charger_on == 1)?"IN":"OUT");
	return 0;
}

#if HIMAX_RESUME_EN

static struct work_struct himax_resume_work;
static struct workqueue_struct *himax_resume_workqueue = NULL;

static void  himax_resume_func(struct work_struct *work)
{
...
void himax_resume_queue_work(void)
{
    cancel_work_sync(&himax_resume_work);
    queue_work(himax_resume_workqueue, &himax_resume_work);
}

int himax_resume_init(void)
{
    INIT_WORK(&himax_resume_work, himax_resume_func);
    himax_resume_workqueue = create_workqueue("himax_resume_wq");
	return 0;
}

int himax_resume_exit(void)
{
    destroy_workqueue(himax_resume_workqueue);
    return 0;
}
#endif
...
int himax_input_register(struct himax_ts_data *ts)
{
	...
	set_bit(GESTURE_DOUBLECLICK, ts->input_dev->keybit);
#endif
...
static int himax_parse_wake_event(struct himax_ts_data *ts)
{
	...
	for(i=0;i<16;i++)
		ts->gesture_cust_en[i]=1;//set to 0 if not use, Rocky

	ts->gesture_cust_en[2] = 0;
	ts->gesture_cust_en[3] = 0;
	ts->gesture_cust_en[4] = 0;
	ts->gesture_cust_en[7] = 0;
	ts->gesture_cust_en[8] = 0;
	ts->gesture_cust_en[13] = 0;
	ts->gesture_cust_en[14] = 0;
	ts->gesture_cust_en[15] = 0;
...
void himax_coord_report(struct himax_ts_data *ts)
{
		...
		if((!tpd_key && !tpd_key_old) || AA_press)
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_ITO_TEST_X00H)
