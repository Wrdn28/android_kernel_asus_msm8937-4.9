...
#if defined(HX_TP_SELF_TEST_DRIVER) || defined(CONFIG_TOUCHSCREEN_HIMAX_ITO_TEST_X00H)
...
extern unsigned char	IC_TYPE_HX;
...
int himax_hand_shaking(struct i2c_client *client)    //0:Running, 1:Stop, 2:I2C Fail
{
	...
	ret = i2c_himax_read(client, 0x90, hw_reset_check, 1, HIMAX_I2C_RETRY_TIMES);
	if (ret < 0) {
	...
	if ((IC_STATUS_CHECK != hw_reset_check[0])) {
		msleep(2);
		ret = i2c_himax_read(client, 0x90, hw_reset_check_2, 1, HIMAX_I2C_RETRY_TIMES);
		if (ret < 0) {
			E("[Himax]:i2c_himax_read 0x90 failed line: %d \n",__LINE__);
			goto work_func_send_i2c_msg_fail;
		}
...
void himax_get_raw_data(uint8_t diag_command, uint16_t mutual_num, uint16_t self_num)
{
	...
	for (i = 0; i < 200; i++) //check for max 1000 times
	{
...
void himax_touch_information(struct i2c_client *client)
{
	...
	if(IC_TYPE_HX == HX_85XX_ES_SERIES_PWON)
		{
			data[0] = 0x8C;
			data[1] = 0x14;
			...
			i2c_himax_master_write(client, &data[0],2,DEFAULT_RETRY_CNT);
#ifdef HX_FIX_MAX_PT
			ic_data->HX_MAX_PT = HX_MAX_PT_VALUE;
#endif
			msleep(10);
			...
			ic_data->HX_RX_NUM				= 14;
			ic_data->HX_TX_NUM				= 26;
			ic_data->HX_BT_NUM				= 2;
			...
			ic_data->HX_XY_REVERSE			= false;
			ic_data->HX_INT_IS_EDGE			= true;
			I("%s:Fix touch info: HX_RX_NUM =%d,HX_TX_NUM =%d,HX_MAX_PT=%d \n", __func__,ic_data->HX_RX_NUM,ic_data->HX_TX_NUM,ic_data->HX_MAX_PT);
			I("%s:Fix touch info: HX_X_RES =%d,HX_Y_RES=%d,HX_INT_IS_EDGE=%d \n", __func__,ic_data->HX_X_RES,ic_data->HX_Y_RES, ic_data->HX_INT_IS_EDGE);
...
bool himax_ic_package_check(struct i2c_client *client)
{
	...
	if(cmd[0] == 0x05 && cmd[1] == 0x85 &&
		...
		{
			IC_TYPE_HX         		= HX_85XX_ES_SERIES_PWON;
